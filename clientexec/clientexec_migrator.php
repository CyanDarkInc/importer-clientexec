...
class ClientexecMigrator extends Migrator
{
    ...
    protected function importCurrencies()
    {
        ...
        foreach ($currencies as $currency) {
            $vars = [
                ...
                'prefix' => $currency->symbol != '' ? $currency->symbol : substr($currency->abrv, 0, 1) . '$',
                'exchange_rate' => $currency->rate,
                'exchange_updated' => null
            ];
            $this->local->duplicate('format', '=', $vars['format'])->duplicate('prefix', '=', $vars['prefix'])->duplicate('exchange_rate', '=', $vars['exchange_rate'])->insert('currencies', $vars);
        }
        $this->local->commit();
    ...
    private function addServiceFields($service, $mapping)
    {
        ...
        foreach ($mapping['service_fields'] as $key => $field) {
            ...
            if (isset($field->callback)) {
                $value = call_user_func_array($field->callback, [$value]);
            }
    ...
    protected function importSupportDepartments()
    {
        ...
        foreach ($departments as $department) {
            ...
            foreach ($staff as $memeber) {
                ...
                $this->local->duplicate('staff_id', '=', $this->mappings['staff'][$memeber->member_id])->insert('support_staff_departments', $vars);
    ...
    {
        $format = '#' . $currency->thousandssep . '###';

        if ($currency->precision > 0) {
            $format = $format . $currency->decimalssep . '##';
        }
